...
public class Webstats {
     ...
     * delimiters.
     *
     * @param line A character array containing the current server log entry
     * line
     * @param delim: A character array containing the delimiters to be used to
     * separate fields in the line
    ...
    public ProcessFileThread multi_thread_process_file(String fileName)
    {
        ProcessFileThread thread = new ProcessFileThread(fileName);
        thread.start();
        return thread;
    }
    ...
    public static void main(String argv[]) throws InterruptedException {
        ...
        List<ProcessFileThread> threads = new ArrayList<>();

        for (int i = 0; i < argv.length; i++) {
            /*process the ith file*/
            threads.add(ws.multi_thread_process_file(argv[i]));
        }
        
        for (ProcessFileThread thread : threads)
            thread.join();
        
//        for (ProcessFileThread thread : threads)
//            combine_webstats(ws, thread.getWebstats());

        ws.print_webstats();
        System.exit(0);
    }
    
    private static void combine_webstats(Webstats main, Webstats secondary)
    {
        main.failed_gets += secondary.failed_gets;
        main.local_bytes += secondary.local_bytes;
        main.local_failed_gets += secondary.local_failed_gets;
        main.local_gets += secondary.local_gets;
        main.requests += secondary.requests;
        main.total_bytes += secondary.total_bytes;
        main.total_gets += secondary.total_gets;
    }
