...
/**
 * This metho creates an empty list with size 0 and no head and tail pointer
 */
				   ...
				   int maxChainLength)
{
	...
	list->head = NULL;
	list->tail = NULL;
	list->maxChainLength = maxChainLength;
	list->getKey = getKey;
...
void freeList(ListPtr list) {
	...
	if (list->size == 1 && list->head != NULL ) {
		freeNode(list->head, freeHashObject);
		free(list);
		return;
	}
	...
	while (temp->next != NULL ) {
		...
		temp = temp->next;
		freeNode(tempPrev, freeHashObject);
	}
	freeNode(temp, freeHashObject);
	free(list);
	return;
}
...
NodePtr search(ListPtr list, void * searchFor, Boolean (*compare)(void *, void *))
{
	...
	NodePtr temp = list->head;
	while (temp != NULL ) {
		if ((*compare)(searchFor, temp->data))
			return temp;
		else
			temp = temp->next;
	}
